...
static void suffstats_off_lookup_triangle(Params * params, GList * srcs, GList * dsts, Counts * counts);
...
gdouble suffstats_logprob_on(gpointer pcounts, Params * params) {
...
gdouble suffstats_logprob_off(gpointer pcounts, Params * params) {
	Counts * counts = pcounts;
...
gpointer suffstats_off_lookup(Params * params, GList * srcs, GList * dsts) {
	...
	suffstats_off_lookup_triangle(params, srcs, dsts, counts);
	if (!dataset_is_symmetric(params->dataset)) {
		/* see how the other half live. */
		suffstats_off_lookup_triangle(params, dsts, srcs, counts);
	}
	return counts;
}
